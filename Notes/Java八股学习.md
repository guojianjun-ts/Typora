...
​	`ReentrantLock` 是指可重入锁，同一个线程在外层方法获得锁时，再进入到这个线程中的内层方法时会自动获取锁，不会因为之前获取过锁但还没释放而阻塞，一定程度可以避免死锁问题。

#### ReentrantLock的实现原理？

​	 `ReentrantLock` 的底层依赖于 AQS，通过组合的模式将锁委托给内部类 `Sync` 继承自AQS。

1. 首先 `ReentrantLock` 继承父类 AQS，AQS 维护了一个同步状态 `status` 来计入重入次数，`status` 初始值为0，
2. 
...
| :----------: | :--------------------------: | :---------------------------: |
| **实现方式** |          API调用AQS          |           依赖于JVM           |
| **锁的获取** | 可非阻塞尝试、可中断、可超时 |      阻塞获取，不可中断       |
|  **公平性**  |     支持公平锁和非公平锁     |          仅非公平锁           |
| **条件变量** |      支持多个Condition       | 仅一个等待队列（wait/notify） |
|  **锁释放**  |    必须手动调用`unlock()`    | 自动释放（代码块结束或异常）  |
|   **性能**   |       高竞争场景下更优       |       低竞争场景下更优        |
